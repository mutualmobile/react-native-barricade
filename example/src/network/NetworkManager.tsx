...
export class NetworkManager {
	...
	appRequest = async <T,>(options: AxiosRequestConfig) => {
		Object.consoleLog("request -> ", options);
	...
	onSuccess: <T>(
		...
		Object.consoleLog("response.data-> ", result);
	...
	onError = async (error: AxiosError<ErrorResponse>) => {
		...
		if (error.response) {
			...
			Object.consoleLog("Error Result:", errorResult);

			title = Strings.errorMessage.oops;
			message =
				errorResult.data?.message ||
				Strings.errorMessage.something_went_wrong;
